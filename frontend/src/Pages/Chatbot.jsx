...
const Bot = () => {
    ...
    const search = async () => {
        if (!file || file.length === 0) {
            ...
            try {
                const processResponse = await axios.post('http://localhost:5000/process_pdfs', formData);
                if (processResponse.status === 200) {
                    const response = await axios.post('http://localhost:5000/ask_question', { question: input });
                    console.log("Response: ", response.data.response);
                    setConversations([...conversations, { input: input, answer: response.data.response }]);
                    setInput("");
                }
            } catch (error) {
                if (error.response) {
        ...
        <>
            <div className={Chat.botcontainer}>
                ...
                <FileUploader
                    ...
                    types={fileTypes}
                />
                <p>{file.length > 0 ? `File name: ${file[0].name}` : "No file uploaded yet"}</p>
                <div className={Chat.inputbox}>
                    ...
                    <button onClick={search}><FontAwesomeIcon beat icon={faMagnifyingGlass} /></button>
                </div>
                ...
                <div>
                    {conversations.slice().reverse().map((chat, index) => (
                        <div key={index}>
                            <p><b>Question : </b>{chat.input}</p>
                            <p><b>Response : </b>{chat.answer}</p>
                        </div>
                    ))}
                </div>
            </div>
